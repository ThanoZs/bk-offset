/**
 * LocationSection.jsx — Premium location and contact info for BK Offset Printing.
 * Features an interactive Google Maps embed, "copy-to-clipboard" functionality,
 * and a status-aware working hours schedule.
 */
...
/**
 * LocationSection — Main layout for the physical address and operational hours.
 */
export function LocationSection({ text, c, isMobile }) {
  ...
  // Surface and text colors based on the current theme context
  ...
  /**
   * handleCopy — Copies the primary address to the system clipboard.
   */
  const handleCopy = () => {
  ...
  return (
    <>
      ...
      >
        ...
        <div style={styles.header(isMobile, isVisible)}>
          <div className="loc-eyebrow" style={styles.eyebrow}>
            <MapPin size={11} strokeWidth={2} style={styles.mrNeg2} />
            Find Us
          </div>
          <h2 style={styles.heading(isMobile, textPri)}>
            Visit Our{" "}
            <em style={styles.headingItalic}>
              Press
            </em>
          </h2>
          <p style={styles.subtext(textDim)}>
        ...
        {/* ── Visual Grid Layout ── */}
        <div style={styles.layoutGrid(isMobile, isVisible)}>
          ...
          <div style={styles.infoCol}>
            ...
            <div
              className="loc-card"
              style={styles.card(borderC, surfBg, "#0ea5e9")}
            >
              <div style={styles.cardRow}>
                {/* Visual marker with a rhythmic pulse effect */}
                <div style={styles.markerContainer}>
                  <div className="loc-ping" style={styles.ping1} />
                  <div className="loc-ping" style={styles.ping2} />
                  <MapPin size={20} strokeWidth={1.75} color="#0ea5e9" className="loc-pin-icon" />
                </div>
                <div style={styles.flex1}>
                  <div style={styles.label("#0ea5e9")}>
                    Address
                  </div>
                  <p style={styles.addressText(textPri)}>
                    {ADDRESS}
                  </p>
                  {/* Clipboard CTA */}
                  <button
                    ...
                    onClick={handleCopy}
                    style={styles.copyBtn(copied, textDim)}
                  >
                    {copied
            ...
            >
              ...
              <div style={styles.hoursList}>
                {HOURS.map(({ day, time, open }) => (
                  <div key={day} className="loc-hours-row" style={styles.hoursRow(borderC)}>
                    <span style={styles.dayLabel(textDim)}>
                      {day}
                    </span>
                    <span style={styles.timeLabel(open, textPri)}>
            ...
            >
              ...
              Get Directions
              <ExternalLink size={13} strokeWidth={2} style={styles.opacity70} />
            </a>
          ...
          <div
            ...
            style={styles.mapWrapper(mapHov, borderC)}
          >
            <iframe
...
const styles = {
  ...
  card: (borderC, surfBg, accent) => ({
    "--loc-grad": `linear-gradient(90deg,${accent},#6366f1)`,
    "--loc-border": borderC,
    background: surfBg,
  ...
  markerContainer: { position: "relative", flexShrink: 0, marginTop: 2 },
  ping1: { width: 32, height: 32, top: -6, left: -6 },
  ping2: { width: 32, height: 32, top: -6, left: -6, animationDelay: "0.7s" },
  flex1: { flex: 1 },
  label: (color, mb = 6) => ({
    fontFamily: "'DM Sans', sans-serif",
    fontSize: "9.5px",
  ...
  copyBtn: (copied, textColor) => ({
    color: copied ? "#22c55e" : textColor,
    background: copied ? "rgba(34,197,94,0.08)" : "transparent",
  }),
  cardHeaderRow: { display: "flex", alignItems: "center", gap: 10, marginBottom: 14 },
  iconBox: (accent) => ({
    width: 34, height: 34,
    borderRadius: 10,
  ...
  hoursList: { "--loc-border": "rgba(0,0,0,0.08)" },
  hoursRow: (borderC) => ({ "--loc-border": borderC }),
  dayLabel: (textColor) => ({
    fontFamily: "'DM Sans', sans-serif",
    color: textColor,
    fontWeight: 400,
  }),
  timeLabel: (open, textColor) => ({
    fontFamily: "'DM Sans', sans-serif",
    fontWeight: 600,
    color: open ? textColor : "#ef4444",
    fontSize: "12.5px",
    display: "flex",
    alignItems: "center",
    gap: 5,
  ...
  dirBtn: { justifyContent: "center" },
  opacity70: { opacity: 0.7 },
  mapWrapper: (mapHov, borderC) => ({
  ...
  mapOverlay: { textDecoration: "none" },
};